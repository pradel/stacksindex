...
describe("historical runtime with handlers", () => {
  ...
  test("stops synchronization when initial page next_cursor jumps past endBlock", async () => {
    const contractId = "SP123.token";
    const handledHeights: number[] = [];
    const handler = vi.fn().mockImplementation((event: { block_height: number }) => {
      handledHeights.push(event.block_height);
    ...
    // Should not have fetched page with cursor 200:0:0:0
    const logsCalls = mockRequest.mock.calls.filter(
      (call: any) =>
        (call[0] as string).includes("cursor=200%3A0%3A0%3A0") ||
        (call[0] as string).includes("cursor=200:0:0:0"),
    ...
    // Sync progress should have saved completion for endBlock 150 without cursor 200
    const progress = await syncStore.getSyncProgress({ contractId, chainId: 1 }, { db: testDb.db });
